...
SOURCE = "uoregon_research_html"
PAGE_URL = "https://research.uoregon.edu/plan/find-funding/external-funding-opportunities"
HEADERS = {"User-Agent": "Mozilla/5.0"}


def _extract_summary(html: str) -> tuple[str, str]:
    soup = BeautifulSoup(html, "html.parser")
    title = (soup.title.get_text(strip=True) if soup.title else "UOregon External Funding")
...
def _build_record(html: str) -> dict[str, Any]:
    ...
    record: dict[str, Any] = {
        "source": SOURCE,
        "source_id": "uoregon-external-funding-opportunities",
        "url": PAGE_URL,
        "apply_url": PAGE_URL,
        "title": title,
        "provider_name": "University of Oregon",
        "award_amount_text": None,
        "deadline_text": None,
        "deadline_date": None,
        "description": summary or "University of Oregon external funding opportunities page.",
        "eligibility_text": summary or "See linked opportunities for eligibility.",
        "requirements_text": summary or "See linked opportunities for requirements.",
