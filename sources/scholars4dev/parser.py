...
def _extract_inline_value(text: str, label: str) -> str | None:
    match = re.search(rf"\b{re.escape(label)}:\s*(.*?)(?:\bStudy in:|\bCourse starts|\bNext course starts|$)", text, re.I)
    return _clean_text(match.group(1)) if match else None


def _extract_study_in(text: str) -> str | None:
    match = re.search(r"\bStudy in:\s*(.*?)(?:\bCourse starts|\bNext course starts|$)", text, re.I)
    return _clean_text(match.group(1)) if match else None
        ...
        for m in matches:
            ...
            if mon:
                try:
                    ...
                    pass
        return max(parsed).date().isoformat() if parsed else None
    parsed = []
    for m in matches:
